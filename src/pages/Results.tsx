...
const Results: React.FC = () => {
  ...
  const handleSavePDF = async () => {
    try {
      ...
      const resultsText = `
Emotional Dynamics Indicator™ Results

Name: ${result?.user.firstName} ${result?.user.lastName}
Date: ${new Date().toLocaleDateString()}

Dominant Harmonic State: ${state?.name}
Description: ${state?.description}

Generated by Emotional Dynamics Indicator™
      `.trim();
      ...
      link.href = url;
      link.download = `EDI-Results-${result?.user.firstName}-${new Date().toISOString().split('T')[0]}.txt`;
